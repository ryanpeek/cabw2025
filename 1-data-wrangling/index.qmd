...
We only need to install packages once per R version, but we need to **`load`** packages every session before we can use them.
...
However, a search for “R” can be challenging and still requires understanding what the question (or error) actually is. A few tips:
